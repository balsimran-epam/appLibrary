...
<div class="wrapper row2">
  ...
  <div class="login">
      ...
      <strong><fmt:message key="login.header.wc" var="wc" />
				${wc}</strong> <fmt:message key="login.header.loginmsg" var="loginmsg" />
				${loginmsg}.</h1>
  <form  name="login" action="ControllerServlet" method="post">
       
        <label for="username"><fmt:message key="login.label.username" /> : </label>
        <input type="text" name="userName" class="input-txt" placeholder="Email address" required autofocus><label for="message"><fmt:message key="login.label.message" /></small></label><br>
