...
<body id="top" onkeydown="return (event.keyCode != 116)">
		...
		<nav id="mainav" class="hoc clear">
			<ul class="clear">
				...
				<li><a href="loginPage.jsp"><fmt:message
							key="register.user.login" var="login" /> ${login}</a></li>
				<li><a href="#signup" data-toggle="modal"
					data-target=".bs-modal-sm"><fmt:message
							key="register.user.sign" var="sign" /> ${sign}</a></li>
				<li><a href="#"><fmt:message key="register.user.au"
							var="au" /> ${why}</a></li>
	...
	<div class="container">
		...
		<div class="modal fade bs-modal-sm" id="myModal" tabindex="-1"
			...
			<div class="modal-dialog modal-sm">
				<div class="modal-content" style="width: 445px;">
					...
					<div class="bs-example bs-example-tabs">
						<ul id="myTab" class="nav nav-tabs">
							...
							<li class=""><a href="#signup" data-toggle="tab"><fmt:message
										key="register.user.header1" var="register" /> ${register}:</a></li>
							<li class=""><a href="#why" data-toggle="tab"><fmt:message
										key="register.user.why" var="why" /> ${why}:</a></li>
						</ul>
					</div>
					<div class="modal-body">
						<div id="myTabContent" class="tab-content">
							<div class="tab-pane fade in" id="why">
								<p>
									<fmt:message key="register.user.whyMsg" var="whyMsg" />
									${whyMsg}
								</p>
								<p></p>
							...
							<div class="tab-pane fade active in" id="signup">
								<form name="myForm" class="form-horizontal"
									action="ControllerServlet" method="get">
									<fieldset>
										<!-- Sign Up Form -->
										<!-- Text input-->
										...
										<div class="control-group">
											<label class="control-label" for="reenterpassword"><fmt:message
													key="home.label.reenterpassword" /> :</label>
											<div class="controls">
										...
										<div class="control-group">
											<label class="control-label" for="streetAddress"><fmt:message
													key="home.label.streetAddress" /> :</label>
											<div class="controls">
												<input id="streetAddress" class="form-control"
													name="streetAddress" type="text"
													placeholder="Street Address" class="input-large" required>
											</div>
										</div>
										<div class="control-group">
											<label class="control-label" for="localityAddress"><fmt:message
													key="home.label.localityAddress" /> :</label>
											<div class="controls">
												<input id="localityAddress" class="form-control"
													name="localityAddress" type="text"
													placeholder="Locality Address" class="input-large" required>
											</div>
										</div>
										<br>
										<div align="center">
											<div class="control-group">
												<label class="control-label" for="humancheck"><fmt:message
														key="home.label.humancheck" /> :</label>
												<div class="controls">
													<label class="radio inline" for="humancheck-0"> <input
														type="radio" name="humancheck" id="humancheck-0"
														value="robot" checked="checked">
													<fmt:message key="register.user.robot" var="robot" />
														${robot}
													</label> <label class="radio inline" for="humancheck-1"> <input
														type="radio" name="humancheck" id="humancheck-1"
														value="human">
													<fmt:message key="register.user.human" var="human" />
														${human}
													</label>
												</div>
											</div>
										...
										<div class="control-group">
											...
											<div class="controls">
												...
												<button type="submit" id="confirmsignup"
													...
													style="width: 76px; height: 36px;">${buttonValue}</button>
												<button type="button" class="btn btn-default pull-right"
													data-dismiss="modal" >
													<fmt:message key="register.user.close" var="close" />
													${close}
												</button>
											</div>
										</div>
					...
					<div class="modal-footer" style="float: center"></div>
				</div>
			</div>
		</div>
		...
		<c:if test="${not empty exceptionOccured}">
			<fmt:message key="register.user.msg" var="msg" />
			<center>
				<h3>${msg}</h3>
			</center>
		</c:if>
		...
		<c:if test="${not empty humancheck}">
			...
			<fmt:message key="register.user.humanMsg" var="humanMsg" />
			<center>
				<h3>${humanMsg}</h3>
			</center>
		</c:if>
		...
		<c:if test="${not empty passwordMatch}">
			...
			<fmt:message key="register.user.passMsg" var="passMsg" />
			<center>
				<h3>${passMsg}</h3>
			</center>
		</c:if>
